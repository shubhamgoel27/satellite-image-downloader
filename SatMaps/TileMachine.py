import urllib
import math
from utils.geo import Point, LatLng, LatLngBounds, Projection
from utils.latlong.auto_zoom import get_zoom_level
...
class TileMachine(object):
    def __init__(self, latlong, size, scale, format, maptype, params, logger=None):
        self.logger = logger
        self.size = size
        self.zoom = get_zoom_level(float(latlong.split(",")[0]), float(latlong.split(",")[1])) 
        self.scale = scale
    ...
    def generate_google_static_map_url_from_latlng(self, latlng, **kwargs):
        ...
        return '{0}{1}&{2}'.format(base, urllib.parse.urlencode(params), '&'.join(self.extra_params))
